...
class CdsLog():
    def __init__(self, logdir, logname, level = logging.INFO):
        self.logdir = logdir
        self.logname = logname
        self.level = level
    def new(self):
        logger = logging.getLogger(__name__)
        logger.propagate = False # don't print to console
        logging.basicConfig(level=self.level)
        handler = RotatingFileHandler(os.path.join(self.logdir, self.logname))
...
if __name__ == "__main__":
    logger = CdsLog(logdir = os.path.dirname(__file__), logname = "logging_test.log").new()
    logger.info("test")
